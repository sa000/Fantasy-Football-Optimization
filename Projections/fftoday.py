...
def generate_urls():
	...
	for index, pos_url in enumerate(pos_urls):
		for year in years:
			pos=positions[index]
			url='http://fftoday.com/stats/playerstats.php?Season='+year+'&GameWeek='+'2'+'&PosID='+pos_url+'&LeagueID=1&order_by=FFPts&sort_order=DESC&cur_page='
			print url
			grab_data(url, year, '2', pos)
...
# def clean_headers():
# 	files=os.listdir('Week2/actual_data')
